...
function Register(props) {
  ...
  useEffect(async () => {
    let response = await api.get(`/consoleGames/getConsoles`)
    ...
    return (
      ...
      <div style={props.style} className={classes.consoles}>
        ...
        <div className={classes.divSelect}>
          <Select onChange={changeConsole} required defaultValue='' name={'consoleInformations[' + props.index + ']'} style={{ paddingRight: 50 }} className={classes.input}>
            <option value='' disabled>Selecione</option>
            {
              consoles.map((console) => {
                return <option value={console.id}>{console.description}</option>
              })
            }
          </Select>
        </div>
  ...
  const handleSubmit = async (data) => {
    ...
    if (stage == 3) {
      try {
        let response = await api.post(`/player/registerPlayer`, obj)
          ...
          <div className={classes.div}>
            <Form ref={formRef} onSubmit={handleSubmit}>
              <div className={classes.content}>
                ...
                <Input required name="playerFullName" type="text" placeholder='Informe seu nome' className={classes.input} />
                ...
                <Input required name="playerNickname" maxLength="10" type="text" placeholder='Informe um usuário' className={classes.input} />
                <label className={classes.label}>Nascimento</label>
                <Input required name="playerBirthDate" type="date" max={limit} placeholder='Selecione a data de nascimento' className={classes.input} />
                <label className={classes.label}>Sexo</label>
                <div className={classes.divSelect}>
                  <Select required defaultValue='' name='playerGener' style={{ paddingRight: 50 }} className={classes.input}>
                    <option value='' disabled>Selecione</option>
                ...
                <Input required onChange={handleChangeCpf} name="playerCpf" type="text" maxLength='14' value={cpf} placeholder='Informe seu CPF' className={classes.input} />
